...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Raven network. This means one can have an always-on redecoind instance running, and another local redecoind/redecoin-qt instance which connects to this node and receives blocks from it.
...
Files used during the gitian build process. For more information about gitian, see the [the Raven Core documentation repository](https://github.com/redecoin-core/docs).
